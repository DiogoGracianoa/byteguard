...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
	...
	if (!sound)
	{
		SDL_Log("Error: Sound '%s' not found or failed to load.", soundName.c_str());
	...
	if (availableChannel == -1)
	{
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); ++it)
		{
			if (it->second.mSoundName == soundName)
			{
				...
				SDL_Log("Stopping repeating sound '%s' on channel %d", soundName.c_str(), availableChannel);
				mHandleMap.erase(it);
				break;
			}
		}
	...
	if (availableChannel == -1)
	{
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); ++it)
		{
			...
			{
				availableChannel = it->second.mChannel;
				SDL_Log("Stopping looping sound '%s' on channel %d", it->second.mSoundName.c_str(), availableChannel);
				StopSound(it->first);
				break;
			}
		}
	...
	if (availableChannel == -1 && !mHandleMap.empty())
	{
		...
		SDL_Log("Stopping older sound '%s' on channel %d", oldest->second.mSoundName.c_str(), availableChannel);
...
void AudioSystem::StopSound(SoundHandle sound)
{
	...
	// Verifica se o handle existe no mapa
	auto it = mHandleMap.find(sound);
	if (it == mHandleMap.end())
	{
		SDL_Log("Error: Attempt to stop sound with invalid handle (%s)", sound.GetDebugStr());
		return;
	}
...
void AudioSystem::PauseSound(SoundHandle sound)
{
	...
	if (it == mHandleMap.end())
	{
		SDL_Log("Erro: Tentativa de pausar som com handle inválido (%s)", sound.GetDebugStr());
		return;
	}
...
void AudioSystem::ResumeSound(SoundHandle sound)
{
	...
	if (it == mHandleMap.end())
	{
		SDL_Log("Error: Attempt to resume sound with invalid handle (%s)", sound.GetDebugStr());
		return;
	}
